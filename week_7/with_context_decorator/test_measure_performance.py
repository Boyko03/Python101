...
class MeasurePerformanceTests(unittest.TestCase):
    ...
    def test_get_exceeded_value_returns_correct_value(self):
        meter = MeasurePerformance(self.filename)

        meter.start = time.time() - 1

        self.assertEqual(int(meter.get_exceeded_time()), 1)
    ...
    def test_benchmark_increases_called_counter_when_called(self):
        with MeasurePerformance(self.filename) as meter:
            time.sleep(1)
    ...
    def test_benchmark_with_no_message_uses_benchmarks_count(self):
        with MeasurePerformance(self.filename) as meter:
            time.sleep(1)
    ...
    def test_benchmark_does_not_restart_original_time_if_not_passed(self):
        with MeasurePerformance(self.filename) as meter:
            ...
            self.assertEqual(original_start, meter.original_start)
    ...
    def test_benchmark_preserves_correct_execution_times_if_not_restarted(self):
        with MeasurePerformance(self.filename) as meter:
            ...
            meter.benchmark('test', restart=False)
